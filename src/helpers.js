function getEmails(message) {
  ...
  const uniqueDomains = getDomains(emails).filter(function(item, pos, self) {
...
function jiraConnection(company) {
  ...
  const httpResponse = UrlFetchApp.fetch(
    `https://3one4capital.atlassian.net/rest/api/2/search?jql=${encodeURIComponent(
      `text ~ ${company}`
    )}`,
    options
  );
  if (httpResponse) {
    const rspns = httpResponse.getResponseCode();
      ...
      case 200:
        ...
        for (const id in data.issues) {
          ...
          if (data.issues[id] && data.issues[id].fields) {
            const { key } = data.issues[id];
            const { summary } = data.issues[id].fields;
